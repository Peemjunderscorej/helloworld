...
public class GameLogic {
    ...
    private static BaseCard topCard = null;
    ...
    public boolean isHandPlayable(int playerIndex) {
        for(int i = 0; i < getPlayerHand(playerIndex).size(); i++){
            if(getPlayerHand(playerIndex).get(i).canPlay()){
                return  true;
            }
        }
    ...
    public ArrayList<BaseCard> draw(int amount) {
        ArrayList<BaseCard> DrawCard = null;
        if(amount > getDeck().size()){
            amount = getDeck().size();
        }

        for(int i = 0; i < amount; i++){
            DrawCard.add(getTopCard());
            getDeck().remove(getTopCard());
        }
    return DrawCard;
